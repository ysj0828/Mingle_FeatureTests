...
/// Description: Unlit/MatCap Blend.
///

Shader "CG Framework/Unlit/MatCap Blend" {
	...
	SubShader {
		...
		// ------------------------------------------------------------------
		//  Grab pass (grabs the current screen contents into a texture)
		GrabPass { 
			Name "GrabScreen"
		}

		// ------------------------------------------------------------------
		//  Unlit rendering pass
		Pass {
			// Pass information
			Name "Unlit"
			Tags {
	...
	CustomEditor "CGFUnlitMatCapBlendMaterialEditor"
}