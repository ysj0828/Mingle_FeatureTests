...
namespace Exoa.Cameras
{
    public class CameraBase : MonoBehaviour, ITouchCamera
    {
        ...
        {
            if (scrollWheel == action && Inputs.GetScroll() != 1)
                return true;
            return false;
        }
        ...
        {
            if (twoFingerPinch == action && Inputs.GetFingerCount() == 2)
                return true;
            return false;
        }
        ...
        protected bool IsInputMatching(InputMapFingerDrag action)
        {
            ...
            if (rightClickDrag == action && Input.GetMouseButton(1) && !Application.isMobilePlatform)
                return true;
            if (twoFingerDrag == action && Inputs.GetFingerCount() == 2)
                return true;
            if (oneFingerDrag == action && Inputs.GetFingerCount() == 1 && !Input.GetMouseButton(1) && !Input.GetMouseButton(2))
                return true;
            return false;
        }
        #endregion
        ...
        protected void DebugInputs()
        {
            ...
            dbg = ("GetFingerCount " + Inputs.GetFingerCount());
            ...
            if (debugTxt != null && !string.IsNullOrEmpty(dbg)) debugTxt.text = dbg;
#endif
        }
