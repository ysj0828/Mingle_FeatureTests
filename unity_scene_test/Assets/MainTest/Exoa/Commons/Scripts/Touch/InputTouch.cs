...
namespace Exoa.Touch
{
    ...
    /// You can access this data via InputTouch.Instance.Fingers, or hook into the InputTouch.On___ events.
    ...
    [AddComponentMenu("InputTouch")]
    public partial class InputTouch : MonoBehaviour
    {
        ...
        public static List<InputTouch> Instances = new List<InputTouch>();
        ...
        public static event System.Action<TouchFinger> OnFingerOld;
        ...
        public static event System.Action<TouchFinger> OnFingerTap;
        ...
        public static event System.Action<TouchFinger> OnFingerSwipe;

        /// <summary>This gets fired every frame at least one finger is touching the screen (List = Fingers).</summary>
        public static event System.Action<List<TouchFinger>> OnGesture;
        ...
        public static event System.Action<TouchFinger> OnFingerExpired;

        /// <summary>This gets fired the frame after a finger went up, </summary>
        public static event System.Action<TouchFinger> OnFingerInactive;
        ...
        private static List<TouchFinger> filteredFingers = new List<TouchFinger>(10);
        ...
        public static List<TouchFinger> GetFingers(bool ignoreIfStartedOverGui, bool ignoreIfOverGui, int requiredFingerCount = 0, bool ignoreHoverFinger = true)
        {
        ...
        private static TouchFinger simulatedTapFinger = new TouchFinger();
        ...
        private static HashSet<TouchFinger> missingFingers = new HashSet<TouchFinger>();

        private static List<TouchFinger> tempFingers = new List<TouchFinger>();

        // Read new hardware finger data
        private void PollFingers()
        {
            // Submit real fingers?
            if (useTouch == true && BaseTouchInput.GetTouchCount() > 0)
            {
                for (var i = 0; i < BaseTouchInput.GetTouchCount(); i++)
                {
                    int id; Vector2 position; float pressure; bool set;

                    BaseTouchInput.GetTouch(i, out id, out position, out pressure, out set);
            ...
            if (useHover == true && BaseTouchInput.GetMouseExists() == true)
            {
            ...
            if (useMouse == true && BaseTouchInput.GetMouseExists() == true)
            {
                var mouseSet = false;
                var mouseUp = false;
        ...
        public TouchFinger AddFinger(int index, Vector2 screenPosition, float pressure, bool set)
        {
            ...
            if (recordFingers == true)
            {
                ...
                if (recordLimit > 0.0f)
                {
                    ...
                    {
                        var removeCount = TouchSnapshot.GetLowerIndex(finger.Snapshots, finger.Age - recordLimit);
                ...
                else if (finger.Index == HOVER_FINGER_INDEX)
                {
                    ...
                    {
                        var removeCount = TouchSnapshot.GetLowerIndex(finger.Snapshots, finger.Age - DEFAULT_RECORD_LIMIT);
        ...
        private TouchFinger FindFinger(int index)
        {
            foreach (var finger in Fingers)
            {
